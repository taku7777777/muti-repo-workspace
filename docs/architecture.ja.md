...
## コンテナ化された実行フロー(Phase 0–3、live 検証済み)

コンテナ経路は macOS サンドボックスを Linux のネットワーク名前空間境界で置き換える。
全フェーズが実機で稼働済み — 実行記録は
[devcontainer-status.md](devcontainer-status.md) を参照。誰が何をやり、各境界を何が
越えるのかを1枚にまとめる:

```
[coder devcontainer — caged ネットワーク。proxy 以外に外への経路なし]
harness(決定的な状態機械 — プログラムであり、LLM ではない)
  ├─ PLAN      … 読み取り専用の LLM セッション ──┐
  ├─ IMPLEMENT … 編集可能な LLM セッション        ├─ LLM の全ターンは egress-proxy
  ├─ REVIEW    … 読み取り専用の LLM セッション    │  (Squid allowlist)経由で
  │              (コードを書いた本人ではない       │  api.anthropic.com のみと通信
  │               fresh なセッション)             ┘
  ├─ TEST GATE … 機械の判定(exit code === 0 のみ。モデルの主張は不採用)
  ├─ 人間ゲート ①(plan 承認) ②(publish 承認) … ターミナル越し
...
1. **チェックの担い手は3種類。** diff の REVIEW は独立した LLM(助言であり、決して
   決定者ではない)、TEST GATE は機械(exit code のみ)、coder 内の承認ゲート2つと
   broker の sha タイプゲートは人間。人間の接点は最小だが、決定権はすべて人間にある。
...
3. **すべての失敗は「publish されていない」を意味する。** broker の応答は型付きで、
   ...
   だから harness はその上に安全にシーケンス制御を組める。
